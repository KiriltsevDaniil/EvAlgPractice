...
island_model = IslandModel([
    canonical_factory.build_canonical(fitness_function=lambda x: sum(x) + 1, pop_size=100, gen_len=1000),
...
# Calculate
print(f"Canonical: {canonical.evaluate(T=100000, p_mut=.9, p_gene_mut=.7)}")
print(f"Genitor: {genitor.evaluate(T=500, p_mut=.7, p_gene_mut=.9)}")
